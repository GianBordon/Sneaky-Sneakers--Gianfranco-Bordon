...
const PlayerPage = ({
  ...
  const handleAddToCart = async (shoe) => {
    ...
    try {
      // Crear un producto temporal para el carrito
      const tempProduct = {
        id: `shoe-${Date.now()}`,
        name: `${playerName} - ${shoe.alt.replace('_', ' ').toUpperCase()}`,
        price: 150, // Precio base para zapatillas de jugadores
        image: shoe.src,
        brand: playerName,
        category: 'players'
      };
  ...
  const handleWishlistToggle = (shoe) => {
    const shoeId = `${playerName}-${shoe.alt}`;
                    ...
                    >
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-cyan-600 font-bold text-lg">$150</span>
                          <button
                  ...
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-3xl font-display font-bold text-neutral-800 mb-4">
                        {shoes[selectedShoeIndex]?.alt.replace('_', ' ').toUpperCase()}
                      </h3>
                    ...
                    <div className="bg-gradient-to-r from-cyan-50 to-blue-50 p-6 rounded-2xl">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-2xl font-bold text-neutral-800">$150</span>
                      ...
                      <div className="flex space-x-3">
                        ...
                        <button
                          ...
                          className={`p-3 rounded-lg transition-colors ${
                            isInWishlist(`${playerName}-${shoes[selectedShoeIndex]?.alt}`)
                              ? 'bg-red-500 text-white hover:bg-red-600'
                              : 'bg-white text-neutral-600 hover:bg-neutral-100 border border-neutral-300'
                          }`}
                        >
                          <svg className="w-6 h-6" fill={isInWishlist(`${playerName}-${shoes[selectedShoeIndex]?.alt}`) ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                          </svg>
                        </button>
                      </div>
