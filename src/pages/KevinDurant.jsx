...
const KevinDurant = () => {
  ...
  // Datos específicos de Kevin Durant
  const playerData = {
    name: "Kevin Durant",
    bannerGradient: "bg-gradient-to-br from-blue-600 to-green-500",
    profileImage: "/src/assets/img/jugadores/KD-cut.jpg",
    bannerImage: "/src/assets/img/KD/Foto-Larga-KD.webp",
    achievements: [
      "2x Campeón de la NBA (2017, 2018)",
      "1x MVP de la NBA (2014)",
      "2x MVP de las Finales (2017, 2018)",
      "13x All-Star (2010-2022)",
      "10x All-NBA Team",
      "4x Líder en puntos de la NBA",
      "Rookie del Año (2008)",
      "2x Medalla de Oro Olímpica (2012, 2016)",
      "MVP del All-Star Game (2019)",
      "50-40-90 Club (2013)"
    ],
    shoes: [
      {
        src: "/src/assets/img/KD/KD14.webp",
        alt: "KD14"
      },
      {
        src: "/src/assets/img/KD/KD12.webp",
        alt: "KD12"
      },
      {
        src: "/src/assets/img/KD/KD10.webp",
        alt: "KD10"
      },
      {
        src: "/src/assets/img/KD/KD 9.webp",
        alt: "KD 9"
      },
      {
        src: "/src/assets/img/KD/KD 8.webp",
        alt: "KD 8"
      },
      {
        src: "/src/assets/img/KD/KD1.webp",
        alt: "KD1"
      }
    ]
  };
  ...
  useEffect(() => {
    const loadPlayerData = async () => {
      try {
        ...
        const playerFromDB = await getPlayerById(2); // Asumiendo que KD tiene ID 2
        if (playerFromDB) {
          setPlayer(playerFromDB);
        }
        
        // Cargar zapatillas de KD desde Supabase
        const shoesFromDB = await getProductsByBrand("KD");
  ...
  const handleWishlistToggle = (shoe) => {
    const shoeId = shoe.product ? shoe.product.id : `kd-${shoe.alt}`;
                ...
                <div className="space-y-8 h-full flex flex-col justify-center">
                  <div className="animate-slide-up">
                    ...
                    <p className="text-base md:text-lg text-neutral-600 mb-6">
                      Una carrera llena de éxitos y reconocimientos que lo convierten en uno de los mejores anotadores de la historia.
                    </p>
                  </div>
                      ...
                      <div className="relative overflow-hidden">
                        ...
                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex space-x-2">
                          ...
                          <button
                            ...
                            className="bg-blue-600 text-white p-2 rounded-full hover:bg-blue-700 transition-colors shadow-lg disabled:bg-neutral-400"
                          >
                            {isAddingToCart ? (
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-blue-600 font-bold text-lg">
                            ${shoe.product ? shoe.product.price : 140}
                          </span>
                          <button
                            onClick={() => handleAddToCart(shoe)}
                            disabled={isAddingToCart}
                            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-neutral-400 disabled:cursor-not-allowed text-sm font-semibold"
                          >
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-green-50 p-6 rounded-2xl">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-2xl font-bold text-neutral-800">
                          ${shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.price : 140}
                        </span>
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          disabled={isAddingToCart}
                          className="flex-1 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-neutral-400 disabled:cursor-not-allowed font-semibold"
                        >
                          {isAddingToCart ? 'Agregando...' : 'Agregar al Carrito'}
                        </button>
                        <button
                          ...
                          className={`p-3 rounded-lg transition-colors ${
                            isInWishlist(shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.id : `kd-${shoes[selectedShoeIndex]?.alt}`)
                              ? 'bg-red-500 text-white hover:bg-red-600'
                              : 'bg-white text-neutral-600 hover:bg-neutral-100 border border-neutral-300'
                          }`}
                        >
                          <svg className="w-6 h-6" fill={isInWishlist(shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.id : `kd-${shoes[selectedShoeIndex]?.alt}`) ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                          </svg>
                        </button>
                      </div>
