...
const PaulGeorge = () => {
  ...
  useEffect(() => {
    const loadPlayerData = async () => {
      try {
        ...
        const playerFromDB = await getPlayerById(4); // PG tiene ID 4
        if (playerFromDB) {
          setPlayer(playerFromDB);
        }
        
        // Cargar zapatillas de PG desde Supabase
        const productsFromDB = await getProductsByBrand("PG");
  ...
  const bannerData = {
    gradient: "bg-gradient-to-br from-red-600 to-blue-500",
    image: "/src/assets/img/PG/paul-george-fotologo.jpg"
  };
