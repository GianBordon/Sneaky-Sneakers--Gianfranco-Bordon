...
const LeBronJames = () => {
  ...
  useEffect(() => {
    const loadPlayerData = async () => {
      try {
        ...
        const playerFromDB = await getPlayerById(1); // LeBron tiene ID 1
        if (playerFromDB) {
          setPlayer(playerFromDB);
        }
        
        // Cargar zapatillas de LeBron desde Supabase
        const productsFromDB = await getProductsByBrand("LeBron");
  ...
  const bannerData = {
    gradient: "bg-gradient-to-br from-purple-600 to-orange-500",
    image: "/src/assets/img/LeBron/lebron-james-with-black-and-white-background-and-lakers-jersey-wallpaper-1920x1080_48.jpg"
  };
