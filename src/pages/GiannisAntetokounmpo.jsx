...
const GiannisAntetokounmpo = () => {
  ...
  // Datos específicos de Giannis Antetokounmpo
  const playerData = {
    name: "Giannis Antetokounmpo",
    bannerGradient: "bg-gradient-to-br from-green-600 to-blue-500",
    profileImage: "/src/assets/img/jugadores/gianis-cut.jpg",
    bannerImage: "/src/assets/img/Giannis/fondo-de-patalla-gianis.webp",
    achievements: [
      "1x Campeón de la NBA (2021)",
      "2x MVP de la NBA (2019, 2020)",
      "1x MVP de las Finales (2021)",
      "9x All-Star (2017-2023)",
      "8x All-NBA Team",
      "7x All-Defensive Team",
      "Defensive Player of the Year (2020)",
      "Most Improved Player (2017)",
      "2x MVP del All-Star Game (2021, 2023)",
      "Líder en rebotes de la NBA (2020)"
    ],
    shoes: [
      {
        src: "/src/assets/img/Giannis/Giannis Zoom Freak 4 Letter Bros.webp",
        alt: "Giannis Zoom Freak 4 Letter Bros"
      },
      {
        src: "/src/assets/img/Giannis/Giannis Zoom Frak 4.webp",
        alt: "Giannis Zoom Frak 4"
      },
      {
        src: "/src/assets/img/Giannis/Giannis Immortality 2.webp",
        alt: "Giannis Immortality 2"
      },
      {
        src: "/src/assets/img/Giannis/Giannis Frak 4.webp",
        alt: "Giannis Frak 4"
      }
    ]
  };
  ...
  useEffect(() => {
    const loadPlayerData = async () => {
      try {
        ...
        const playerFromDB = await getPlayerById(3); // Asumiendo que Giannis tiene ID 3
        if (playerFromDB) {
          setPlayer(playerFromDB);
        }
        
        // Cargar zapatillas de Giannis desde Supabase
        const shoesFromDB = await getProductsByBrand("Giannis");
  ...
  const handleWishlistToggle = (shoe) => {
    const shoeId = shoe.product ? shoe.product.id : `giannis-${shoe.alt}`;
    ...
    <>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${playerData.bannerImage})`,
                ...
                <div className="space-y-8 h-full flex flex-col justify-center">
                  <div className="animate-slide-up">
                    ...
                    <p className="text-base md:text-lg text-neutral-600 mb-6">
                      Una carrera llena de éxitos y reconocimientos que lo convierten en uno de los jugadores más dominantes de la historia.
                    </p>
                  </div>
                      ...
                      <div className="relative overflow-hidden">
                        ...
                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex space-x-2">
                          ...
                          <button
                            ...
                            className="bg-green-600 text-white p-2 rounded-full hover:bg-green-700 transition-colors shadow-lg disabled:bg-neutral-400"
                          >
                            {isAddingToCart ? (
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-green-600 font-bold text-lg">
                            ${shoe.product ? shoe.product.price : 130}
                          </span>
                          <button
                            onClick={() => handleAddToCart(shoe)}
                            disabled={isAddingToCart}
                            className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:bg-neutral-400 disabled:cursor-not-allowed text-sm font-semibold"
                          >
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-2xl">
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-2xl font-bold text-neutral-800">
                          ${shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.price : 130}
                        </span>
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          disabled={isAddingToCart}
                          className="flex-1 bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition-colors disabled:bg-neutral-400 disabled:cursor-not-allowed font-semibold"
                        >
                          {isAddingToCart ? 'Agregando...' : 'Agregar al Carrito'}
                        </button>
                        <button
                          ...
                          className={`p-3 rounded-lg transition-colors ${
                            isInWishlist(shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.id : `giannis-${shoes[selectedShoeIndex]?.alt}`)
                              ? 'bg-red-500 text-white hover:bg-red-600'
                              : 'bg-white text-neutral-600 hover:bg-neutral-100 border border-neutral-300'
                          }`}
                        >
                          <svg className="w-6 h-6" fill={isInWishlist(shoes[selectedShoeIndex]?.product ? shoes[selectedShoeIndex].product.id : `giannis-${shoes[selectedShoeIndex]?.alt}`) ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                          </svg>
                        </button>
                      </div>
